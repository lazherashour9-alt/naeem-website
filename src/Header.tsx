    ...
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-sm border-b border-border shadow-sm">
      <div className="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <div className="md:hidden flex items-center gap-3">
          <button onClick={() => setMobileOpen(!mobileOpen)} className="p-2 rounded-md border border-border hover:bg-secondary">
            {mobileOpen ? <X size={20} /> : <Menu size={20} />}
          </button>
          <button onClick={toggleLanguage} className="flex items-center gap-2 px-3 py-1 rounded-sm border border-border hover:bg-secondary">
            <Globe size={16} className="text-accent" />
            <span className="text-sm font-medium">{language === 'en' ? 'AR' : 'EN'}</span>
          </button>
        </div>
