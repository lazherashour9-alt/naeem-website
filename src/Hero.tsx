...
import heroImage from './assets/hero-custom.png'; // relative import
  ...
  return (
    <section id="home" onPointerMove={handlePointerMove} className="relative isolate flex min-h-screen w-full items-center justify-center overflow-hidden bg-neutral-950 text-white">
      ...
      <div className="relative z-10 mx-auto w-full max-w-4xl px-4 text-center">
        <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="mx-auto max-w-4xl text-5xl font-black text-white sm:text-6xl md:text-7xl lg:text-8xl">
          <span className="bg-clip-text text-transparent bg-gradient-to-b from-white via-white to-white/75">{content.hero.title}</span>
        </motion.h1>

        <motion.p initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="mx-auto mt-6 max-w-2xl text-base text-white/80 sm:text-lg md:text-xl">
          {content.hero.subtitle}
        </motion.p>

        <div className="mt-10 flex justify-center">
          <motion.button onClick={handleCTA} whileHover={{ scale: 1.04, y: -2 }} whileTap={{ scale: 0.98 }} className="group relative inline-flex items-center justify-center rounded-full border border-white/15 bg-white px-8 py-3.5 font-semibold text-slate-950 shadow-[0_18px_60px_rgba(255,255,255,0.18)] transition-shadow duration-300 hover:shadow-[0_22px_80px_rgba(255,255,255,0.26)]">
        ...
        <motion.button type="button" aria-label="Scroll to products" onClick={handleCTA} className="absolute bottom-7 left-1/2 z-10 -translate-x-1/2 rounded-full border border-white/15 bg-white/10 px-4 py-3 text-white/90 backdrop-blur-md shadow-lg" animate={mounted ? { y: [0, 8, 0] } : {}} transition={{ duration: 1.6, repeat: Infinity, ease: "easeInOut" }} whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.96 }}>
          <ChevronDown size={26} />
        </motion.button>
      </div>
    </section>
